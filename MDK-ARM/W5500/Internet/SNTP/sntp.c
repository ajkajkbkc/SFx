...
void get_seconds_from_ntp_server(uint8_t *buf, uint16_t idx)
{
	...
	LOGW("sntp", "get sntp time seconds is : %llu", seconds);
	switch (time_zone)
	{
	case 0:
		seconds -=  12*3600;
...
{
#if 0  //当seconds过大时，会进入下面的while死循环
	uint8_t yf=0;
...
#else
	rtc_datetime_t datetime_st;
	ConvertSecondsToDatetime(seconds, &datetime_st, 0);

	Nowdatetime.yy = datetime_st.year;
	Nowdatetime.mo=datetime_st.month;
	Nowdatetime.dd=datetime_st.day;
	Nowdatetime.hh = datetime_st.hour;
	Nowdatetime.mm = datetime_st.minute;
	Nowdatetime.ss = datetime_st.second;
#endif
}
